use blake2b::{Blake2b, Hash, ParameterBlock};
...
fn initialise_state(n: u32, k: u32, digest_len: u8) -> Blake2b {
    ...
    let params = ParameterBlock::new()
        .set_digest_len(digest_len)
        .set_key_len(0)
        .set_fanout(1)
        .set_max_depth(1)
        .set_personalization(&personalization);
    Blake2b::with_parameter_block(digest_len as usize, params)
}

fn generate_hash(base_state: &Blake2b, i: u32) -> Hash {
    let mut lei = vec![];
...
pub fn is_valid_solution_iterative(
    ...
    for i in indices {
        ...
        rows.push(Node {
            hash: expand_array(&hash[start..end], p.collision_bit_length(), 0),
            indices: vec![*i],
        });
    }
