...
use zcash_primitives::{merkle_tree::IncrementalWitness, transaction::components::Amount};
...
impl Account {
    ...
    pub(crate) fn select_notes(
        &self,
        value: Amount,
    ) -> Result<Vec<(Diversifier, Note<Bls12>, IncrementalWitness)>, Error> {
        let notes: Vec<(Diversifier, Note<Bls12>, IncrementalWitness)> = self
            .notes
            .iter()
            .filter_map(|n| match n.tx_spent {
                Some(_) => None,
